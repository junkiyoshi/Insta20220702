...
void ofApp::update() {
	...
	for (int i = 0; i < this->location_list.size(); i++) {
		...
		if (ofNoise(this->location_list[i].x * 0.008, this->location_list[i].y * 0.008, this->location_list[i].z * 0.008 + ofGetFrameNum() * 0.03) < 0.5) {
...
void ofApp::setBoxToMesh(ofMesh& face_target, ofMesh& frame_target, glm::vec3 location, float height, float width, float depth) {
	...
	for (int i = 0; i < 8; i++) {
		...
		face_target.addColor(ofColor(255, ofMap(location.z, -100, 100, 0, 255)));
		frame_target.addColor(ofColor(0, ofMap(location.z, -100, 100, 0, 255)));
	}
}
